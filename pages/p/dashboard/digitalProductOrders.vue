            ...
            <div class="grid flex items-center justify-start w-full grid-cols-6">
              ...
              <div class="col-span-6 lg:col-span-4">
                ...
                <div v-if="data.subsets_Digital">
                  <div v-for="(subset_Digital, index) in data.subsets_Digital" :key="index">
                    <div class="flex items-center justify-center flex-wrap py-3">
                      <div v-for="cell in subset_Digital.data" :key="cell.id" class="px-5 flex flex-col items-center">
                        <p class="font-bold">{{ cell.title }}</p>
                        <p>{{ cell.body }}</p>
                      </div>
                    </div>
...
export default {
  ...
  methods: {
    ...
    hasValidInstructions(data) {
      const ins = data.digital_product.instructions.trim();
      return ins !== '<p class="ql-align-right ql-direction-rtl">null</p>'
          && ins !== '<p class="ql-align-right ql-direction-rtl"><br></p>'
          && ins !== 'null' && ins !== '';
    },
    toggleInstructions(id) {
      const idx = this.showInstructionsId.indexOf(id);
      if (idx > -1) this.showInstructionsId.splice(idx, 1);
      else this.showInstructionsId.push(id);
    },
    ...
    getData() {
      ...
      axios.get(`${apiStore().address}/api/order/order-digital-product-user-list/`, {
        ...
        // assume DRF pagination response
