<template>
    <div class="fixed mb-3 bottom-0 left-0 z-50 grid w-full h-16 grid-cols-1 px-8 md:grid-cols-3 ">
        <div class="items-center bg-glass-3 rounded-full px-3 justify-center hidden ml-auto text-indigo-600 md:flex">
        <form>   
            <div class="relative">
                ...
                <input type="search" v-model="inputValue" @focus="onFocus" @blur="onBlur" id="default-search" class="w-full p-4 pl-10 text-sm text-indigo-600 rounded-full bg-transparent focus:border-transparent placeholder-indigo-600  border-transparent" placeholder="Search..." required>
            </div>
        </form>
    </div>
        ...
        <div class="flex items-center bg-glass-3 px-8 rounded-full justify-center mx-auto">
            ...
            <button data-tooltip-target="tooltip-camera" type="button" class="p-2.5 transform hover:-translate-y-3  duration-500 bg-indigo-600 group rounded-full hover:bg-indigo-800 mr-4 focus:outline-none focus:ring-4 focus:ring-indigo-200 ">
                <UserIcon  class=" w-5 text-white" />
            </button>
         
            <button data-tooltip-target="tooltip-feedback" type="button" class="p-2.5 transform hover:-translate-y-3  duration-500 bg-indigo-600 group rounded-full hover:bg-indigo-800 mr-4 focus:outline-none focus:ring-4 focus:ring-indigo-200 ">
                <ShoppingCartIcon class=" w-5 text-white" />
            </button>
        
            <button data-tooltip-target="tooltip-settings" type="button" class="p-2.5 animate-pulse transform hover:-translate-y-3  duration-500 bg-indigo-600 group rounded-full mr-4 md:mr-0 hover:bg-indigo-800 focus:outline-none focus:ring-4 focus:ring-indigo-200 ">
        ...
        <div class=" bg-glass-3 px-8 rounded-full items-center justify-center hidden mr-auto md:flex">
            <div class="flex">
                <div v-for="page in pages" :key="page.name" class="rounded-full mx-1 duration-500 hover:bg-indigo-600 text-indigo-900 hover:text-white hover:border-indigo-600  border-indigo-400 border py-4">
...
<script>
import { Bars3Icon, HomeIcon, ShoppingCartIcon, UserIcon, BellAlertIcon } from '@heroicons/vue/24/solid'

export default {
    components:{UserIcon,HomeIcon,ShoppingCartIcon,BellAlertIcon},
  data() {
    return {
      inputValue: '',
      pages: [
...
input {
  ...
  outline: none; /* Remove the outline (focus ring) */
