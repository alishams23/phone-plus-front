    ...
    <div class="container mx-auto">
      <div class="md:rounded-3xl isolate overflow-hidden relative border border-b" v-if="loading == false">
          ...
          <div class="flex pt-3 justify-between items-center">
            <div class="rtl px-3 pt-3">
              <button @click="$router.push('/p/search/?sort_digital_product=-rate&tab=1');"
                class="text-black text-sm mg:text-lg rtl cursor-pointer">
                بیشتر
        ...
        <nuxt-link :to="'/p/digitalProduct/' + digital_products[index].id"
          class="flex flex-col md:flex-row justify-center md:justify-between container mx-auto">
          <img :src="digital_products[index].image[0].photo" alt=""
            class="md:mb-10 me-4 md:mx-20 mt-10 md:mt-0 mx-4 md:w-[200px] h-[140px] md:h-[200px] border shadow-2 object-cover rounded-3xl md:w-auto" />
      ...
      <div class="flex justify-center -mt-8 mb-4 me-4">
        <button @click="prevItem" class="text-black text-sm cursor-pointer me-2 h-10 w-10 flex justify-center items-center bg-indigo-200 text-indigo-800 rounded-full ">
          <ChevronLeftIcon class="w-8 h-8" />
        </button>
        <button @click="nextItem" class="text-black text-sm cursor-pointer ms-2 h-10 w-10 flex justify-center items-center bg-indigo-200 text-indigo-800 rounded-full ">
