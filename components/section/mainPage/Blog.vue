...
<div class="py-[60px] bg-white flex items-center justify-center mt-20 mx-3" v-if="loading">
  ...
  <div v-else>
    <div class="flex  mt-[-20px] justify-between items-center">
      <div class="rtl mx-10">
        <a href="#" class="rtl text-sm mg:text-lg  flex items-center">
          بیشتر
          <span aria-hidden="true" class="px-3 pt-1 "> &larr;</span>
        </a>
      </div>

      <div class="font-bold text-md md:text-xl text-right px-10  flex items-center justify-end">مقاله ها
      ...
      <div class="mx-auto max-w-7xl px-6 md:px-8">
        ...
        <div class="mx-auto grid w-2xl grid-cols-1 gap-x-10 gap-y-10 md:mx-0 md:max-w-none md:grid-cols-2">
          <div v-for="(blog, index) in blogs" :key="blog.id">
            <Blog class="md:px-7 pt-10 md:pt-0" :blog="blog" data-aos="fade-up" data-aos-anchor-placement="top-bottom"
